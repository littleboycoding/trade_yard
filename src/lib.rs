...
mod tests {
    ...
    async fn program_test_setup() -> (BanksClient, Keypair, Hash, (Keypair, Keypair, Keypair)) {
        ...
        let setup_instructions = [
            ...
            create_associated_token_account(&payer.pubkey(), &payer.pubkey(), &mint.pubkey()),
            create_associated_token_account(&payer.pubkey(), &buyer.pubkey(), &mint.pubkey()),
            mint_to(
                &spl_token::id(),
                &mint.pubkey(),
                &payer_nft_wallet,
            ...
            .unwrap(),
            create_associated_token_account(&payer.pubkey(), &payer.pubkey(), &payment.pubkey()),
            create_associated_token_account(&payer.pubkey(), &buyer.pubkey(), &payment.pubkey()),
            mint_to(
                &spl_token::id(),
                &payment.pubkey(),
                &buyer_wallet,
